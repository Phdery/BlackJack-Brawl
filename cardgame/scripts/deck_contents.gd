...
func _ready() -> void:
	self.visible = true
	#self.label.global_position = Vector2(10, 50)
	#self.button.global_position = Vector2(50, 50)
	cards = deck.cards
	# test code for adding card children
	var test_card_1 = basic_card_scene.instantiate()
	test_card_1.custom_init(2,"heart")
	test_card_1.scale = Vector2(0.25, 0.25)
	test_card_1.global_position = Vector2(500, 500)
...
func display_cards() -> void:
	...
	for card in cards:
		...
		if (i % num_per_row == 0):
			y += 1
		card.global_position = start_position + Vector2(200 * (i % num_per_row), 100 * y)
		i += 1
